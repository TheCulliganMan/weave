...
type PanelInteractMode = 'edit' | 'export-report' | null;
...
export const useSetInteractingPanel = () => {
  const {setState} = usePanelInteractContext();
  return useCallback(
    (panelInteractMode: PanelInteractMode, path: string[]) => {
      setState(prevState => ({
        ...prevState,
        panelInteractMode,
        selectedPath: path,
...
export const useSetInteractingChildPanel = () => {
  ...
  return useCallback(
    (panelInteractMode: PanelInteractMode, childPath: string) => {
      setInteractingPanel(panelInteractMode, path.concat([childPath]));
    },
    [path, setInteractingPanel]
  );
};

export const useClosePanelInteractDrawer = () => {
  const {setState} = usePanelInteractContext();
  return useCallback(() => {
    setState(prevState => ({
      ...prevState,
